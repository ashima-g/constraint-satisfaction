    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
            	...
            	constraint_list.append(AllDiffConstraint("square", scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for plane in planes_problem.planes:
    	vars = []
    	vars.append(plane)
    	for i in range(len(planes_problem.can_fly(plane))):
    		scope = planes_problem.can_fly(plane)
        	vars.append(Variable('{}{}'.format(plane, i), scope))
        	indices['{}{}'.format(plane,i)] = (k,i+1)
        planes.append(vars)
        k+=1
    ...
    for i in range(len(planes)): 
    	...
    	if K<=len(can_fly):
    		for j in range(len(can_fly)):
    			if K+j > len(can_fly):
    				break
	    		cons.append(NValuesConstraint("maintenance{}".format(currentPlane), flights[j:j+K], planes_problem.maintenance_flights, 1, K))
    	...
    	#narrow down the possible flights after the first one 
		
		#for every flight in the previous variable's domain, figure out flights that can follow and add to the scope		
	
        leadFlight = [x[0] for x in scope] 
        temp = set(leadFlight)
        leadFlight = list(temp) #removes duplicates
        
        startFlight = legal_start.intersection(leadFlight)
        startDomain = list(startFlight) 
        
        for k in range(len(flights)-1):
        	totalNextDomain = []
        	satAssignments = []
        	for s in startDomain: 
		    	nextDomain = [v[1] for v in scope if v[0]==s]
		    	satAssignments.extend([[s,n] for n in nextDomain])
		    	
		    	if nextDomain != []:
		    		totalNextDomain.extend(nextDomain)	
            
    		startDomain = totalNextDomain
    		cons.append(TableConstraint("canFollow", [flights[k],flights[k+1]], satAssignments))
    		totalNextDomain = []
    		satAssignments = []
    		
    	allFlights.extend(flights)
    	
	#all flights can only be scheduled once 
	cons.append(AllDiffConstraint("flights", allFlights))
    
    csp = CSP("Planes", allFlights, cons)
    ...
    for solution in solutions: 
    	totalPlanes = []
    	for k in range(len(planes_problem.planes)): 
    		newSol = []
    		newSol.append(planes_problem.planes[k])
    		newSol.extend([x[1] for x in solution if indices[x[0].name()][0]==k])
    		totalPlanes.append(newSol)
    		
    	newResult.append(totalPlanes)
    return newResult
    #FINAL SOLUTION
