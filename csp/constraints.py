...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for v1 in qi.domain():
        	for v2 in qj.domain():
        		if (abs(v1-v2)==difference) or v1==v2:
        			continue
        		else: 
        			satisfyingAssignments.append([v1,v2])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
            if var.isAssigned():
                assignment = var.getValue()
                if assignment in self._required: 
                	count+=1	
    ...
    def hasSupport(self, var, val):
        ...
        #if var=val and there exists an assignment to the other variables 
        #that satisfy the assignment then return true
        
        if var not in self.scope():
            return True   

        def requirementSatisfied(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the all diff'''
            vals = [val for (var, val) in l]
            total=0 
            for requiredVal in self._required:
				total+= vals.count(requiredVal)
            return (total>=self._lb and total<=self._ub)
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], requirementSatisfied)
        return x
